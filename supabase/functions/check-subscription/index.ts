...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader) {
      return new Response(JSON.stringify({ subscribed: false, status: 'no_auth' }), {
    ...
    if (userError) {
      ...
      return new Response(JSON.stringify({ subscribed: false, status: 'auth_error' }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
    }
    ...
    if (!user?.email) {
      ...
      return new Response(JSON.stringify({ subscribed: false, status: 'no_user' }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });
    }
    ...
    return new Response(JSON.stringify({ 
      error: "Unable to verify subscription. Please try again or contact support." 
    }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 500,
    });
